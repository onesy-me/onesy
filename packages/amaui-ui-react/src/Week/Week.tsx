...
export interface IWeek extends ILine {
  date?: AmauiDate;
...
const Week: React.FC<IWeek> = React.forwardRef((props_, ref: any) => {
  const theme = useAmauiTheme();

  const props = React.useMemo(() => ({ ...theme?.ui?.elements?.all?.props?.default, ...theme?.ui?.elements?.amauiWeek?.props?.default, ...props_ }), [props_]);
  ...
  const timesUI = React.useCallback((day: any, index: number) => {
    // clean up
    refs.days.current = {};
    ...
    return <>
      {times.map(item => <>
        {/* Weekly */}
        {renderTimes(day, (item.weekly!.days as any)![index + 1]?.values as any, true, (item.weekly!.days as any)![index + 1])}

        {/* Dates */}
        {renderTimes(day, getDates(item), false)}
      </>)}
    </>;
  }, [theme, times, date]);
