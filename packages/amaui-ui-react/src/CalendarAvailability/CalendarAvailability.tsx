...
import { cleanValue, elementToCanvas, is, textToInnerHTML } from '@amaui/utils';
...
const CalendarAvailability: React.FC<ICalendarAvailability> = React.forwardRef((props_, ref: any) => {
  ...
  const onPrevious = React.useCallback(() => {
    ...
    setDate(previous => {
      valueNew = remove(1, 'week', previous);
  ...
  const formattedDate = React.useMemo(() => {
    return `${format(startOf(date, 'week'), `MMM DD, YYYY`)} – ${format(endOf(date, 'week'), `MMM DD, YYYY`)}`;
  }, [view, date]);
