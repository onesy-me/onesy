import React from 'react';

import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialTvWithAssistantW100Filled = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='TvWithAssistantW100Filled'

      short_name='TvWithAssistant'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M420-472q11 0 19.5-8.5T448-500q0-11-8.5-19.5T420-528q-11 0-19.5 8.5T392-500q0 11 8.5 19.5T420-472Zm120 0q11 0 19.5-8.5T568-500q0-11-8.5-19.5T540-528q-11 0-19.5 8.5T512-500q0 11 8.5 19.5T540-472Zm-240 0q11 0 19.5-8.5T328-500q0-11-8.5-19.5T300-528q-11 0-19.5 8.5T272-500q0 11 8.5 19.5T300-472Zm360 0q11 0 19.5-8.5T688-500q0-11-8.5-19.5T660-528q-11 0-19.5 8.5T632-500q0 11 8.5 19.5T660-472ZM192-200v-52h-60v-496h696v496h-60v52h-14l-19-52H227l-21 52h-14Z"/>
    </Icon>
  );
});

IconMaterialTvWithAssistantW100Filled.displayName = 'OnesyIconMaterialTvWithAssistantW100Filled';

export default IconMaterialTvWithAssistantW100Filled;
