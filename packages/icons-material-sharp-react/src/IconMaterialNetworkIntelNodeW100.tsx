import React from 'react';

import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialNetworkIntelNodeW100 = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='NetworkIntelNodeW100'

      short_name='NetworkIntelNode'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M346-212q-8 0-15-3.5T320-227l-68-123h48l36 68h77v-28h-60l-36-68h-81l-49-87q-2-4-3-7.5t-1-7.5q0-2 4-15l49-87h81l36-68h60v-28h-77l-36 68h-48l68-123q4-8 11-11.5t15-3.5h90q13 0 21.5 8.5T466-718v140h-61l-28 28h89v106h-76l-34-67h-69l-28 28h80l34 67h93v174q0 13-8.5 21.5T436-212h-90Zm214 0q-23 0-38.5-15.5T506-266q0-19 11.5-33t28.5-19v-324q-17-5-28.5-19T506-694q0-23 15.5-38.5T560-748q23 0 38.5 15.5T614-694q0 19-11.5 33T574-642v120l108-65q-8-26 8.5-47t43.5-21q23 0 38.5 15.5T788-601q0 23-15.5 38.5T734-547q-11 0-20-4.5T697-563l-112 68 117 93q7-5 15-7.5t17-2.5q23 0 38.5 15.5T788-358q0 23-15.5 38.5T734-304q-30 0-45.5-25t-4.5-51l-110-88v151q17 5 28 18.5t11 32.5q0 23-15 38.5T560-212Z"/>
    </Icon>
  );
});

IconMaterialNetworkIntelNodeW100.displayName = 'OnesyIconMaterialNetworkIntelNodeW100';

export default IconMaterialNetworkIntelNodeW100;
