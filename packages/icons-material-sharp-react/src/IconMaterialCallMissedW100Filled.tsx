import React from 'react';

import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialCallMissedW100Filled = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='CallMissedW100Filled'

      short_name='CallMissed'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M485-300 200-585v220h-28v-268h268v28H220l265 265 284-284 20 20-304 304Z"/>
    </Icon>
  );
});

IconMaterialCallMissedW100Filled.displayName = 'OnesyIconMaterialCallMissedW100Filled';

export default IconMaterialCallMissedW100Filled;
