import React from 'react';

import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialShieldToggleW100 = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='ShieldToggleW100'

      short_name='ShieldToggle'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M420.9-521h117.8q24.3 0 41.8-17.26 17.5-17.26 17.5-41.5t-17.3-41.74Q563.4-639 539.1-639H421.3q-24.3 0-41.8 17.26-17.5 17.26-17.5 41.5t17.3 41.74q17.3 17.5 41.6 17.5Zm123.14-33q-11.04 0-18.54-7.46-7.5-7.47-7.5-18.5 0-11.04 7.46-18.54 7.47-7.5 18.5-7.5 11.04 0 18.54 7.46 7.5 7.47 7.5 18.5 0 11.04-7.46 18.54-7.47 7.5-18.5 7.5ZM420.9-361h117.8q24.3 0 41.8-17.26 17.5-17.26 17.5-41.5t-17.3-41.74Q563.4-479 539.1-479H421.3q-24.3 0-41.8 17.26-17.5 17.26-17.5 41.5t17.3 41.74q17.3 17.5 41.6 17.5Zm-4.86-33q-11.04 0-18.54-7.46-7.5-7.47-7.5-18.5 0-11.04 7.46-18.54 7.47-7.5 18.5-7.5 11.04 0 18.54 7.46 7.5 7.47 7.5 18.5 0 11.04-7.46 18.54-7.47 7.5-18.5 7.5ZM480-139q-5 0-11-1t-11.02-3Q345-188 278.5-291.5 212-395 212-516v-166q0-19.26 10.88-34.66Q233.75-732.07 251-739l208-77q11-4 21-4t21 4l208 77q17.25 6.93 28.13 22.34Q748-701.26 748-682v166q0 121-66.5 224.5T502.02-143q-5.02 2-11.02 3t-11 1Zm0-25q104-33 172-132t68-220v-167q0-10-5.5-18T699-713l-208-77q-5-2-11-2t-11 2l-208 77q-10 4-15.5 12t-5.5 18v167q0 121 68 220t172 132Zm0-314Z"/>
    </Icon>
  );
});

IconMaterialShieldToggleW100.displayName = 'OnesyIconMaterialShieldToggleW100';

export default IconMaterialShieldToggleW100;
