import React from 'react';

import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialBikeScooterW100 = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='BikeScooterW100'

      short_name='BikeScooter'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M12-226v-28h186q6-72 57-125t119-59l-62-284q-2-11-11.5-17.5T282-746H172q-5.95 0-9.98-4.04-4.02-4.03-4.02-10 0-5.96 4.02-9.96 4.03-4 9.98-4h110q18 0 35.5 12.5T340-728l68 316h-8q-71.77 0-122.89 51.11Q226-309.77 226-238v12H12Zm388.12 82q-39.12 0-66.62-27.38-27.5-27.38-27.5-66.5 0-39.12 27.38-66.62 27.38-27.5 66.5-27.5 39.12 0 66.62 27.38 27.5 27.38 27.5 66.5 0 39.12-27.38 66.62-27.38 27.5-66.5 27.5Zm-.12-28q27 0 46.5-19.5T466-238q0-27-19.5-46.5T400-304q-27 0-46.5 19.5T334-238q0 27 19.5 46.5T400-172Zm372-96q-69 0-117.5-43.5T598-426H486l-6-28h118q3-29 22.5-71.5T678-588H452l-6-28h246l-70-192q-3-9-10-13.5t-16-4.5h-64q-5.95 0-9.97-4.04-4.03-4.03-4.03-10 0-5.96 4.03-9.96 4.02-4 9.97-4h64q17 0 31.35 9.69Q641.7-834.62 648-818l74 202h50q74 0 124 50t50 124q0 71-52.5 122.5T772-268Zm-.04-26Q834-294 876-337q42-43 42-103.4 0-61.6-41.97-103.6-41.98-42-104.03-42-11 0-21 .5t-19 3.5l37 99q2 5-.33 10.39-2.34 5.38-7.67 7.61-5 2-10.42-.33-5.41-2.34-7.58-7.67l-35-99q-39 20-60.5 57T626-440q0 60 41.96 103 41.97 43 104 43ZM400-238Zm372-202Z"/>
    </Icon>
  );
});

IconMaterialBikeScooterW100.displayName = 'OnesyIconMaterialBikeScooterW100';

export default IconMaterialBikeScooterW100;
