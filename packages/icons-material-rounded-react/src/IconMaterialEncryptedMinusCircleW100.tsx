import React from 'react';

import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialEncryptedMinusCircleW100 = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='EncryptedMinusCircleW100'

      short_name='EncryptedMinusCircle'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M212-516v-166q0-19.26 10.88-34.66Q233.75-732.07 251-739l208-77q11-4 21-4t21 4l208 77q17.25 6.93 28.13 22.34Q748-701.26 748-682v140q0 5.95-4.04 9.97-4.03 4.03-10 4.03-5.96 0-9.96-4.03-4-4.02-4-9.97v-141q0-10-5.5-18T699-713l-208-77q-5-2-11-2t-11 2l-208 77q-10 4-15.5 12t-5.5 18v167q0 55 14.5 105.5T295-317q26 43 61 77t76 56q5 3 7 8t-1 10q-3 5-8 7t-10-1q-97-52-152.5-149.5T212-516Zm268 37Zm120 213h159.62q6.38 0 10.38-4.2 4-4.2 4-9.8 0-5.6-4.2-9.8-4.2-4.2-9.8-4.2H600.38q-6.38 0-10.38 4.2-4 4.2-4 9.8 0 5.6 4.2 9.8 4.2 4.2 9.8 4.2Zm80 134q-62 0-105-43t-43-105q0-62 43-105t105-43q62 0 105 43t43 105q0 62-43 105t-105 43ZM479.9-508q21.91 0 37.5-15 15.6-15 15.6-36.9 0-21.91-15.6-37.5-15.59-15.6-37.5-15.6-21.9 0-36.9 15.6-15 15.59-15 37.5 0 21.9 15 36.9t36.9 15Z"/>
    </Icon>
  );
});

IconMaterialEncryptedMinusCircleW100.displayName = 'OnesyIconMaterialEncryptedMinusCircleW100';

export default IconMaterialEncryptedMinusCircleW100;
