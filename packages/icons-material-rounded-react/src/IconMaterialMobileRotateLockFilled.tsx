import React from 'react';

import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialMobileRotateLockFilled = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='MobileRotateLockFilled'

      short_name='MobileRotateLock'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M714-600q-14 0-24-10t-10-24v-132q0-14 10-24t24-10h6v-40q0-33 23.5-56.5T800-920q33 0 56.5 23.5T880-840v40h6q14 0 24 10t10 24v132q0 14-10 24t-24 10H714Zm46-200h80v-40q0-17-11.5-28.5T800-880q-17 0-28.5 11.5T760-840v40ZM401-87l-77-77q-11-11-11-28t11-28q11-11 28-11t28 11L550-50q12 12 6.5 28.5T534-3q-14 2-27 2.5T480 0q-99 0-186.5-37.5t-153-103Q75-206 37.5-293.5T0-480q0-17 11.5-28.5T40-520q17 0 28.5 11.5T80-480q0 71 24 136t66.5 117Q213-175 272-138.5T401-87Zm-28-469q13 0 21.5-9t8.5-21q0-13-8.5-21.5T373-616q-12 0-21 8.5t-9 21.5q0 12 9 21t21 9Zm122.97 374L182.03-496Q171-507 165.5-521.5 160-536 160-550q0-15 5.5-29t16.54-25.06l173.61-173.88Q366.69-789 381.25-795q14.55-6 28.6-6 15.05 0 29.1 6t25.08 17.03l313.94 313.74Q789-453.2 795-439.17q6 14.03 6 29.07 0 14.03-6 28.57Q789-367 777.96-356L604.35-182q-11.04 11-25.09 16.5t-29.11 5.5q-14.05 0-28.6-5.5T495.97-182Z"/>
    </Icon>
  );
});

IconMaterialMobileRotateLockFilled.displayName = 'OnesyIconMaterialMobileRotateLockFilled';

export default IconMaterialMobileRotateLockFilled;
