import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialMobileLockLandscapeW100Filled = (props: IIcon) => {

  return (
    <Icon
      name='MobileLockLandscapeW100Filled'

      short_name='MobileLockLandscape'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M152-252q-26 0-43-17t-17-43v-340q0-24.75 17.63-42.38Q127.25-712 152-712h130q0-14 9.5-25t23.5-11h68q14 0 23.5 11t9.5 25h392q24.75 0 42.38 17.62Q868-676.75 868-652v340q0 24.75-17.62 42.37Q832.75-252 808-252H152Zm255.94-86h143.78q15.28 0 25.78-10.35Q588-358.7 588-374v-104.23q0-14.77-9.78-25.27Q568.45-514 554-514h-5v-40q0-29-19.5-48.5T481-622q-29 0-48.5 19.5T413-554v40h-5q-15.3 0-25.65 10.35Q372-493.3 372-478v104.23q0 14.77 10.33 25.27 10.34 10.5 25.61 10.5ZM441-514v-40q0-17 11.5-28.5T481-594q17 0 28.5 11.5T521-554v40h-80Z"/>
    </Icon>
  );
};

IconMaterialMobileLockLandscapeW100Filled.displayName = 'OnesyIconMaterialMobileLockLandscapeW100Filled';

export default IconMaterialMobileLockLandscapeW100Filled;
