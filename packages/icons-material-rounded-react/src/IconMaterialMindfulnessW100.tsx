import React from 'react';

import { Icon, IIcon } from '@onesy/ui-react';

const IconMaterialMindfulnessW100 = React.forwardRef((props: IIcon, ref) => {

  return (
    <Icon
      ref={ref}

      name='MindfulnessW100'

      short_name='Mindfulness'

      viewBox='0 -960 960 960'

      {...props}
    >
      <path d="M480.04-426q5.96 0 9.96-4.02 4-4.03 4-9.98v-200q0-5.95-4.04-9.97-4.03-4.03-10-4.03-5.96 0-9.96 4.03-4 4.02-4 9.97v200q0 5.95 4.04 9.98 4.03 4.02 10 4.02Zm120-60q5.96 0 9.96-4.02 4-4.03 4-9.98v-100q0-5.95-4.04-9.97-4.03-4.03-10-4.03-5.96 0-9.96 4.03-4 4.02-4 9.97v100q0 5.95 4.04 9.98 4.03 4.02 10 4.02Zm-240-20q5.96 0 9.96-4.02 4-4.03 4-9.98v-80q0-5.95-4.04-9.97-4.03-4.03-10-4.03-5.96 0-9.96 4.03-4 4.02-4 9.97v80q0 5.95 4.04 9.98 4.03 4.02 10 4.02ZM292-276q-57-52-88.5-115.5T172-520.72q0-128.04 89.83-217.66Q351.67-828 480-828q103 0 187.5 63.5T777-601l38.32 151.92Q819-435 810-423.5q-9 11.5-24 11.5h-78v140q0 24.75-17.62 42.37Q672.75-212 648-212H548v66q0 5.95-4.04 9.98-4.03 4.02-10 4.02-5.96 0-9.96-4.02-4-4.03-4-9.98v-63.51q0-13.49 8.63-21.99 8.62-8.5 21.37-8.5h98q14 0 23-9t9-23v-138q0-12.75 8.63-21.38Q697.25-440 710-440h78l-38-155q-23-92-99-148.5T480-800q-116 0-198 81.29T200-521q0 59.74 24.5 113.51T294-311.9l26 23.9v142q0 5.95-4.04 9.98-4.03 4.02-10 4.02-5.96 0-9.96-4.02-4-4.03-4-9.98v-130Zm202-190Z"/>
    </Icon>
  );
});

IconMaterialMindfulnessW100.displayName = 'OnesyIconMaterialMindfulnessW100';

export default IconMaterialMindfulnessW100;
